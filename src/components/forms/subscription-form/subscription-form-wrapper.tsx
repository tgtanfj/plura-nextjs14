...
const SubscriptionFormWrapper = ({ customerId, planExists }: Props) => {
  ...
  const [selectedPriceId, setSetlectedPriceId] = useState<Plan | "">(
  ...
  useEffect(() => {
    ...
    if (planExists) {
      toast({
        title: "Success",
        description: "You plan has been successfully upgraded!",
      });
      setClose();
      router.refresh();
    }
    ...
    <div className="border-none transition-all">
      <div className="flex flex-col gap-4">
        {data.plans?.plans.map((price) => (
          <Card
          onClick={() => setSetlectedPriceId(price.id as Plan)}
            className={clsx("realatice cursor-pointer transition-all", {
